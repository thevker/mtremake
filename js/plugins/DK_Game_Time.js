...
/*:
 * @plugindesc v.2.8.0 [MV|MZ] Time system.
 * @author DKPlugins
 ...
 * @help
 ...
 ## Control of switches
 ###===========================================================================
 You can control switches based on time and date,
 for this use the "Switches" section in the plugin settings.
 It is NOT recommended to use the section "Switches (deprecated)" !!!
 In the settings, specify the switch and the condition for its activation.
 The condition is set using JavaScript.
 The result of the calculations must be written to the "enabled" variable.

 Examples:
 1. Condition for turning on the switch from 10:30 to 12:00
 enabled = (hour == 10 && min >= 30 || hour == 11);

 2. Condition for turning on the switch on days off
 enabled = $gameTime.isDayOff();

 3. Condition for turning on the switch on weekdays
 enabled = !$gameTime.isDayOff();

 4. Condition for turning on the switch on mondays in october
 enabled = (dayWeek == Game_Time.MONDAY && month == Game_Time.OCTOBER);

 5. Condition for turning on the switch in the morning
 enabled = dayPeriod == Game_Time.MORNING;

 You can see all the constants below in the help!
 ...
 19. Run common event over time:
 GameTimeRunCommonEvent eventId type value
 eventId - Common event ID. Calculated with Javascript.
 type - Parameter (sec, min, hour, day, month, year)
 value - Parameter value. Calculated with Javascript.
 Example1: GameTimeRunCommonEvent 1 min 5
 Example2: GameTimeRunCommonEvent $gameVariables.value(1) hour $gameVariables.value(2)

 20. Remove change timer*:
 * - created with commands GameTimeChangeSwitch, GameTimeChangeSelfSwitch, GameTimeChangeVariable, GameTimeRunCommonEvent or script calls
 ...
 21. Add temporary state to actor:
 GameTimeAddActorState actorId stateId type value
 actorId - Actor ID. Calculated with Javascript.
 stateId - State ID. Calculated with Javascript.
 type - Parameter (sec, min, hour, day, month, year)
 value - Parameter value. Calculated with Javascript.
 Example1: GameTimeAddActorState 1 1 min 5
 Example2: GameTimeAddActorState $gameVariables.value(1) $gameVariables.value(2) hour $gameVariables.value(3)
 ...
 Game_Time.addActorState(actorId, stateId, gameTime) - Add temporary state to actor. When $gameTime >= gameTime, state will be removed.
 ...
 * @command GameTimeRunCommonEvent
 * @desc Run common event over time
 *
 * @arg eventId
 * @text Common event
 * @desc Common event. Calculated with Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 ...
 * @command GameTimeAddActorState
 * @desc Add temporary state to actor
 *
 * @arg actorId
 * @text Actor ID
 * @desc Actor ID. Calculated with Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 ...
 * @arg stateId
 * @text State ID
 * @desc State ID. Calculated with Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 ...
 * @param hideMapWindowWhenMessage
 * @text Hide during messages
 * @parent mapWindow
 * @desc Hide window during messages ?
 * @type boolean
 * @default false

 * @param hideMapWindowWhenEvent
 * @text Hide during events
 * @parent mapWindow
 * @desc Hide window during events ?
 * @type boolean
 * @default false

 * @param hideMapWindowWhenMovement
 * @text Hide during movement
 * @parent mapWindow
 * @desc Hide window during player movement ?
 * @type boolean
 * @default false
...
/*:ru
 * @plugindesc v.2.8.0 [MV|MZ] Система времени.
 * @author DKPlugins
 * @url https://dk-plugins.ru
 * @target MZ
 * @help
 ...
 ###=========================================================================
 Вы можете управлять переключателями на основе времени и даты,
 для этого используйте раздел "Переключатели" в настройках плагина.
 НЕ рекомендуется использовать раздел "Переключатели (устарело)" !!!
 В настройках укажите переключатель и условие его активации.
 Условие задается с помощью JavaScript.
 Результат вычислений обязательно нужно записать в переменную "enabled".

 Примеры:
 1. Условие для включения переключателя с 10:30 до 12:00
 enabled = (hour == 10 && min >= 30 || hour == 11);

 2. Условие для включения переключателя по выходным
 enabled = $gameTime.isDayOff();

 3. Условие для включения переключателя по будням
 enabled = !$gameTime.isDayOff();

 4. Условие для включения переключателя по понедельникам в октябре
 enabled = (dayWeek == Game_Time.MONDAY && month == Game_Time.OCTOBER);

 5. Условие для включения переключателя по утрам
 enabled = dayPeriod == Game_Time.MORNING;

 Посмотреть все константы вы можете ниже в справке!
 ...
 19. Запустить общее событие через время:
 GameTimeRunCommonEvent eventId type value
 eventId - Номер общего события. Вычисляется с помощью Javascript.
 type - Параметр (sec, min, hour, day, month, year)
 value - Значение параметра. Вычисляется с помощью Javascript.
 Пример1: GameTimeRunCommonEvent 1 min 5
 Пример2: GameTimeRunCommonEvent $gameVariables.value(1) hour $gameVariables.value(2)

 20. Удалить таймер изменения*:
 * - созданные с помощью команд GameTimeChangeSwitch, GameTimeChangeSelfSwitch, GameTimeChangeVariable, GameTimeRunCommonEvent или вызовами скриптов
 RemoveGameTimeChangeTimer ID
 ...
 21. Добавить временное состояние для персонажа:
 GameTimeAddActorState actorId stateId type value
 actorId - Номер персонажа. Вычисляется с помощью Javascript.
 stateId - Номер состояния. Вычисляется с помощью Javascript.
 type - Параметр (sec, min, hour, day, month, year)
 value - Значение параметра. Вычисляется с помощью Javascript.
 Пример1: GameTimeAddActorState 1 1 min 5
 Пример2: GameTimeAddActorState $gameVariables.value(1) $gameVariables.value(2) hour $gameVariables.value(3)
 ...
 Game_Time.addTimer(gameTime, action) - Добавить таймер. Когда $gameTime будет >= gameTime, выполнится действие action
 Game_Time.remTimer(id) - Удалить таймер
 Game_Time.addActorState(actorId, stateId, gameTime) - Добавить временное состояние для персонажа. Когда $gameTime >= gameTime, состояние будет удалено.
 ...
 Game_Time.JANUARY - Январь (месяц)
 Game_Time.FEBRUARY - Февраль (месяц)
 Game_Time.MARCH - Март (месяц)
 Game_Time.APRIL - Апрель (месяц)
 Game_Time.MAY - Май (месяц)
 Game_Time.JUNE - Июнь (месяц)
 Game_Time.JULY - Июль (месяц)
 Game_Time.AUGUST - Август (месяц)
 Game_Time.SEPTEMBER - Сентябрь (месяц)
 Game_Time.OCTOBER - Октябрь (месяц)
 Game_Time.NOVEMBER - Ноябрь (месяц)
 Game_Time.DECEMBER - Декабрь (месяц)
 Game_Time.WINTER - Зима (сезон)
 Game_Time.SPRING - Весна (сезон)
 Game_Time.SUMMER - Лето (сезон)
 Game_Time.AUTUMN - Осень (сезон)
 ...
 * @command GameTimeRunCommonEvent
 * @desc Запустить общее событие через время
 *
 * @arg eventId
 * @text Номер общего события
 * @desc Номер общего события. Вычисляется с помощью Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 ...
 * @command GameTimeAddActorState
 * @desc Добавить временное состояние для персонажа
 *
 * @arg actorId
 * @text Номер персонажа
 * @desc Номер персонажа. Вычисляется с помощью Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 *
 * @arg stateId
 * @text Номер состояния
 * @desc Номер состояния. Вычисляется с помощью Javascript.
 * @type combo
 * @option 1
 * @option $gameVariables.value(ID)
 * @default 1
 ...
 * @param hideMapWindowWhenMessage
 * @text Скрывать во время сообщений
 * @parent mapWindow
 * @desc Скрывать окно во время сообщений ?
 * @type boolean
 * @default false

 * @param hideMapWindowWhenEvent
 * @text Скрывать во время событий
 * @parent mapWindow
 * @desc Скрывать окно во время событий ?
 * @type boolean
 * @default false

 * @param hideMapWindowWhenMovement
 * @text Скрывать во время перемещения
 * @parent mapWindow
 * @desc Скрыть окно во время движения игрока ?
 * @type boolean
 * @default false
 ...
 * @param condition
 * @text Condition
 * @desc Your JavaScript code that is responsible for enabling the switch.
 * @type note
 * @default "// write true/false to enabled variable\nenabled = false;"

*/

/*~struct~SwitchEval:ru
 ...
 * @param condition
 * @text Условие
 * @desc Ваш JavaScript код, который отвечает за включение переключателя.
 * @type note
 * @default "// запишите true/false в переменную enabled\nenabled = false;"
...
if (GameTimeParams.switches.length > 0) {
    console.warn('DK_Game_Time: The "Switches (deprecated)" setting is deprecated, use the "Switches" setting instead!');
}
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    switch (command) {
        ...
        case 'GameTimeRunCommonEvent': {
            Game_Time.addTimer($gameTime.clone().add(args[1], eval(args[2])), {
                type: 'commonEvent',
                params: [eval(args[0])]
            });
            break;
        ...
        case 'GameTimeAddActorState': {
            Game_Time.addActorState(eval(args[0]), eval(args[1]), $gameTime.clone().add(args[2], eval(args[3])));
            break;
        }
    }
};
...
if (Utils.RPGMAKER_NAME === 'MZ') {
    ...
    PluginManager.registerCommand('DK_Game_Time', 'GameTimeRunCommonEvent', (args) => {
        Game_Time.addTimer($gameTime.clone().add(args.type, eval(args.value)), {
            type: 'commonEvent',
            params: [eval(args.eventId)]
        });
    });

    PluginManager.registerCommand('DK_Game_Time', 'GameTimeAddActorState', (args) => {
        Game_Time.addActorState(eval(args.actorId), eval(args.stateId), $gameTime.clone().add(args.type, eval(args.value)));
    });
...
class Game_Time {
     ...
     */
    lessDate(gameTime) {
        if (this._year < gameTime.year) {
            return true;
        } else if (this._year > gameTime.year) {
            return false;
        }

        if (this._month < gameTime.month) {
            return true;
        } else if (this._month > gameTime.month) {
            return false;
        }

        return this._day < gameTime.day;
    }
    ...
    lessTime(gameTime, blockSeconds = false) {
        if (this._hour < gameTime.hour) {
            return true;
        } else if (this._hour > gameTime.hour) {
            return false;
        }

        if (this._min < gameTime.min) {
            return true;
        } else if (this._min > gameTime.min) {
            return false;
        }
    ...
    static extractSaveContents(data) {
        ...
        this._actorStates = data.actorStates || [];
     ...
     * @param {Game_Time} gameTime
     */
    static addActorState(actorId, stateId, gameTime) {
        $gameActors.actor(actorId).addState(stateId);
        this._actorStates.push({ actorId, stateId, gameTime });
    }
    ...
    static updateSwitches() {
        ...
        GameTimeParams.switchesEval.forEach((data) => {
            if (data.switch > 0) {
                let enabled = false;

                try {
                    const { sec, min, hour, day, dayWeek, dayPeriod, month, year } = $gameTime;

                    enabled = eval(data.condition);
                } catch(e) {
                }
            ...
            if ($gameTime.moreEquals(timer.gameTime)) {
                switch (timer.action.type) {
                    ...
                    case 'variable':
                        ...
                        break;
                    case 'commonEvent':
                        $gameTemp.reserveCommonEvent(...timer.action.params);
                        break;
                    case 'script':
     ...
     * @static
     */
    static updateActorStates() {
        this._actorStates = this._actorStates.filter((data) => {
            const actor = $gameActors.actor(data.actorId);

            if ($gameTime.moreEquals(data.gameTime)) {
                actor.removeState(data.stateId);
                return false;
            }
...
Scene_Map.prototype.updateGameTimeWindowVisibility = function() {
    this._gameTimeWindow.visible = $gameSystem.isGameTimeWindowVisible() && this.isActive();

    if (GameTimeParams.hideMapWindowWhenMessage && $gameMessage.isBusy()) {
        this._gameTimeWindow.visible = false;
        return;
    }

    if (GameTimeParams.hideMapWindowWhenEvent && $gameMap.isEventRunning()) {
        this._gameTimeWindow.visible = false;
        return;
    }

    if (GameTimeParams.hideMapWindowWhenMovement && $gamePlayer.isMovementSucceeded()) {
        const isMoving = $gamePlayer.isMoving() || Input.dir4 || $gameTemp.isDestinationValid();
