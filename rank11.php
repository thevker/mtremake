...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $player_name = isset($_GET['player_name']) ? $_GET['player_name'] : 0;
    ...
    foreach ($lines as &$line) {
        if (strpos($line, "$au_id") !== false) {
            // 已存在的玩家，更新分数
            $line = sprintf("%s rank_point=%d %s", $au_id, $totalRankPoint, $player_name);
            $found = true;
            break;
        }
    }
    ...
    if (!$found) {
        // 如果没有找到玩家，添加新的行
        $lines[] = sprintf("%s rank_point=%d %s", $au_id, $totalRankPoint, $player_name);
    }
    ...
    if (file_put_contents($filePath, mb_convert_encoding($dataToSave, 'UTF-8')) !== false) {
        echo "数据保存成功，$au_id $rank_point $player_name";
    } else {
        echo "数据保存失败";
    }
}
